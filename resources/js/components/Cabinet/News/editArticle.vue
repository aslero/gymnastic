<template>
    <div>
        ...
        <form method="post" @submit.prevent="editArticle()" enctype="multipart/form-data" class="form-add side">
            <div class="forms-wrapper pr-50">
                        ...
                        <draggable v-model="forms" >
                            <transition-group type="transition" name="flip-list">
                                <div class="form--item" v-for="(item, index) in forms" :key="index">
                                    <div class="item pos-r">
                                        <input type="text" :maxlength="max" v-model="item.title" :aria-valuetext="'item.title'" placeholder="Заголовок (не обязательно)">
                                    ...
                                    <div class="item-upload" v-else-if="item.image == null">
                                        <p class="upload--title">Добавить изображения:</p>
                                        <span class="upload--description">Вы можете загрузить до 5 изображений. Размер каждого не более 2048 Kb</span>
                                        <div class="data-gallery">
                                            <label :for="'gallery_id_'+index" class="gallery_id">
                                    ...
                                    <div class="item-footer">
                                        ...
                                        <button class="btn-blue" type="button" @click.prevent="deleteBlock(item,index)">удалить блок</button>
                                    </div>
                                </div>
                            </transition-group>
                        </draggable>
                    </div>
                    <div class="forms-btn-action">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            deleteBlock(item,x){
                axios.post('/cabinet/edit/post/delete/block',{id:item.id})
                    .then((response) => {
                        if (response.data.error === 0) {
                            this.forms.splice(x, 1);
                        }
                    })
                    .catch(error => {});
            },
            editArticle(){
                ...
                formData.append('articles', JSON.stringify(this.forms));
                ...
                axios.post('/cabinet/edit/post/update',formData)
                    .then((response) => {
                        if (response.data.error == 0) {
                            toast.fire({
                                type: 'success',
            ...
            uploadImages(item,tnum){
                ...
                $(event.target.files).each(function( index,value,form ) { //Идем по массиву загруженных картинок
                    ...
                    return axios.post('/upload/article/images',formData,config) //Загружаем фотку на сервак и присылаем обратно название
                        .then((response) => {
                            if (response.data.error === 0) {
                                let el = that.forms.find((f, idx) => f.image == '' || f.image == null && idx >= tnum);
                                if (el){
